...
class FileActions:
    ...
    def rename_file(
        ...
        try:
            ...
            logger.info(
                rf"Source path renamed to destination path successfully: {destination}"
            )
            return True
    ...
    @staticmethod
    def back_up_rename(
        source: Union[str, _PathLike[str]],
        destination: Union[str, _PathLike[str]],
    ) -> bool:
        try:
    ...
    def change_file_name(
        ...
        try:
            ...
            logger.warning(
                "Failed rename attempt, will try again", extra=dict(error=err)
            )
            try:
                renamed_state = cls.back_up_rename(source, destination)
                logger.info(
                    f"Renamed file: {destination} ",
                    extra=dict(renamed_state=renamed_state),
                )
            except Exception as err:
                logger.error(
                    "Failed rename attempt 2, will not retry", extra=dict(error=err)
                )
                renamed_state = False
    ...
    def delete_file(path: Union[str, bytes, os.PathLike]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete file: {path} because: {err}",
                extra=dict(file_name=path),
            )
            return False
    ...
    def delete_directory(path: Union[str, bytes, os.PathLike]) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to delete folder: {path} because {err}")
            return False
